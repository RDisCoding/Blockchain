...
contract FundMeTest is Test{
    ...
    function testDundsFailWithoutEnoughEth() public {
        vm.expectRevert(); //hey, the next line should revert -> foundry cheatcode
        fundMe.fund(); // This should revert because 0 ETH is sent
    }

    function testFundUpdatesFundedDataStructure() public {
        vm.prank(USER); // foundry cheatcode that the next tx will be sent by USER
    ...
    function testWithdrawWithASingleFunder() public funded {
        ...
        // Act
        uint256 gasStart = gasleft(); //built in function to get the current gas left
        vm.txGasPrice(GAS_PRICE);
        vm.prank(fundMe.getOwner());
        fundMe.withdraw();

        uint256 gasUsed = (gasStart - gasleft()) * tx.gasprice; // calculate the gas used
